...
fetchUserData("www.dropbox.in",function f1(error1,data1){  // Once we have fetched the details from the URL then f1 will be executed. f1 is a callback
    if(error1){
        ...
        fetchPostData(data1,function f2(error2,data2){ // Once we have fetched the posts from the userdata, then f2 will be executed. f2 is a callback.
            if(error2){
                console.log("The error from fetching the posts is",error2);
            }else{
                console.log("The user posts details are",data2);
                fetchCommentData(data2,function f3(error3,data3){ // Once we have fetched the comments from the userPosts, then f3 will be executed, f3 is a callback
                    if(error3){
